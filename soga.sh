...
docker run --restart=on-failure --name v72 -d -v /etc/soga/:/etc/soga/ --network host vaxilu/soga:latest --type=sspanel-uim --server_type=v2ray --api=webapi --webapi_url=https://baidupi.xyz --webapi_key=weilehaoji --node_id=343 --dns_cache_time=10 --soga_key=HNBZvn0GxEvWYKIi2pHUzd118s3dMuGY --forbidden_bit_torrent=true --proxy_protocol=true --force_vmess_aead=true --tunnel_enable=true --tunnel_type=tcp-tunnel --tunnel_password=80a8d91d-9347-40ae-b3a6-abffdf3d61f0
docker run --restart=on-failure --name v231 -d -v /etc/soga/:/etc/soga/ --network host vaxilu/soga --type=sspanel-uim --server_type=trojan --api=webapi --webapi_url=https://baidupi.xyz --webapi_key=weilehaoji --node_id=231 --dns_cache_time=10 --cert_domain=hkt5.bieqiang.xyz --cert_key_length=ec-256 --cert_mode=dns --cert_key_length= --dns_provider=dns_cf --DNS_CF_Email=sethroy987@gmail.com --DNS_CF_Key=a1a9d07704cb75bd6e2dbc62f8728a2fde4b3  --soga_key=yDGbZFgLdDyhlrp1bXR4B50R8JzD8um2
#启动sg
...
docker run $COMMON_OPTS --name v348 $IMAGE --type=sspanel-uim --server_type=v2ray --api=webapi --webapi_url=$WEBAPI_URL --webapi_key=$WEBAPI_KEY --node_id=348 --dns_cache_time=10 --soga_key=$SOGA_KEY --forbidden_bit_torrent=true --force_vmess_aead=true
docker run $COMMON_OPTS --name v106 $IMAGE --type=sspanel-uim --server_type=trojan --api=webapi --webapi_url=$WEBAPI_URL --webapi_key=$WEBAPI_KEY --node_id=106 --dns_cache_time=10 --cert_domain=sga5.bieqiang.xyz --cert_key_length=ec-256 --cert_mode=dns --dns_provider=dns_cf --DNS_CF_Email=sethroy987@gmail.com --DNS_CF_Key=a1a9d07704cb75bd6e2dbc62f8728a2fde4b3 --soga_key=$TROJAN_SOGA_KEY
